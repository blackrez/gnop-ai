...
func getInput() tensor.Tensor {
	...
	img_tmp, _, err := image.Decode(imgF)
	if err != nil {
		log.Fatal(err)
	}
	img = &img_tmp
	...
	var m image.Image
	if (img_tmp.Bounds().Max.X - img_tmp.Bounds().Min.X) > (img_tmp.Bounds().Max.Y - img_tmp.Bounds().Min.Y) {
		scaleFactor = float32(img_tmp.Bounds().Max.Y-img_tmp.Bounds().Min.Y) / float32(hSize)
		m = resize.Resize(0, hSize, img_tmp, resize.Lanczos3)
	} else {
